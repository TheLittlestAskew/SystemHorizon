...
- **Commit:** `e5e335f` (M1) · `cb13d7b` (this handoff) · `5f6d706` (the M2 correction below)
- **🛑 M0's "M2 needs no re-seed" is wrong, and the Projects page is already showing it.** Ran a read-only full-outer-join of live `horizon_projects` against `App.jsx`'s hardcoded registry. All 16 rows are present, none missing, none extra — but **only 8 of 16 match**: 7 carry the wrong `area` and 2 are missing `parent_name`. The live table is still a **pre-split 5-area model** — no `Undercroft`, no `Sidequests`, the four campaign vaults still inside `Aftermath` (7 instead of 3), and a phantom `Swift` area in no code path. Because `AREA_ORDER` renders six cards, **`Undercroft` and `Sidequests` are empty on the Projects page right now** and the 2 `Swift` rows land in the trailing out-of-order section. That is very likely what the long-standing Projects visual-verification item would have caught. Row-by-row table in `NORTH_STAR.md` Q6. ▶ **A row count is not a data-contract check — M0 checked `count(*)`, got 16, and stopped; the count was never the problem.** No fix applied: the seed path already writes both columns and upserts on `(owner, name)`, so re-running it as Taylor corrects all 8 rows, but that needs her login and Q6 now has **two** questions (`Undercroft` vs `The Undercroft`, and whether Fantasy Football is a Side Quest or Learning). **M2 moved to `Blocked`.**
...
- **Answered mid-session (2026-09-25):** Taylor closed **Q5, Q6 and Q8**. Q6: area is **`Undercroft`** and **Fantasy Football is a Side Quest** — both answers mean the code is already right and the database is the stale side, so no code change, and M2 is now blocked on her login alone. Q5/Q8: **retired `systemhorizon-build`** to `~/.claude/skills/_superseded/` (folder moved, 6 files byte-untouched, nothing deleted; added a `README.md` documenting the new convention). 🛑 **Q8 said one dangling pointer; a tree-wide grep found five across four files** — `SKILLS-INDEX.md`, `cynosure/references/brand-hooks.md`, and the **`description:` frontmatter** of both `cynosure/SKILL.md` and `rectrix-caedere-site/SKILL.md`. The two frontmatter ones are the half that actually drives auto-triggering, so a prose-only fix would have left the real problem running. The `rectrix-caedere-site` mentions are disambiguation guards and were **rewritten, not removed** — the two apps really are both React + Supabase. ⚠️ The skills tree is **not** version-controlled, so pre-edit copies of all four files sit in the session scratchpad at `skills-backup-2026-09-25/`. ▶ **Grep the whole tree before declaring a skill retired; I found the second pointer by accident, not by looking.**
- **Next:** Taylor logs into the dev server (Chrome is already open on `localhost:5173`) so M1's gate item 8 can close, then M2's re-seed.
